...
def heuristics(current_pos, end_pos, heuristics_mode):
    if heuristics_mode == "euclidean":
        return sqrt((end_pos[0]-current_pos[0])**2 + (end_pos[1]-current_pos[1])**2)
    if heuristics_mode == "manhattan":
...
def search(maze, start, end, heuristics_mode):
    ...
    start_node.g = 0
    start_node.h = heuristics(start, end, heuristics_mode)
    ...
    while len(yet_to_visit_list) > 0:
        ...
        for child in children:
            ...
            if len([node for node in visited_list if node == child]) > 0:
                continue
