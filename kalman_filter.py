...
class kalman_filter:
    ...
    def update(self, z):
        ...
        # calculate how much we want to rely on the measurement
        kalman_gain=np.dot(np.dot(self.P, self.C.T), np.linalg.inv(S))
        
        # compare actual measurements with expected measurments
        surprise_error= z - self.measurement_model()
        
        # perform mean estimation
        self.x=self.x + np.dot(kalman_gain, surprise_error)

        # estimate covariance matrix adjusting for the information gain from measurement
